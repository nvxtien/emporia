# Emporia Authentication Service
...
exist. Flyway creates the `emporia_authentication` schema and database tables
automatically.
...
```bash
docker build -t emporia-authentication emporia/authentication
docker run --rm -p 9000:9000 \
  -e DB_URL=jdbc:postgresql://host.docker.internal:5433/emporia_authentication \
  -e DB_USERNAME=postgres \
  -e DB_PASSWORD='your-local-database-password' \
  -e AUTH_ISSUER=http://localhost:3000 \
  emporia-authentication
```
...
| `DB_SCHEMA` | `emporia_authentication` | Service-owned PostgreSQL schema |
