...
public class BookBuilderTest {
    ...
    String depthJson = "[{\"symbol\":\"CHK\",\"marketPercent\":0.0292,\"volume\":3743791,\"lastSalePrice\":0.157,\"last"+
            ...
            "reason\":\"    \",\"timestamp\":1583840133820},\"opHaltStatus\":{\"isHalted\":false,\"timestamp\":1583840133820},\"ss" +
    ...
    // ---------------------------------------------------------------------------
    // Additional BookBuilder tests using a programmatically constructed Depth
    // ---------------------------------------------------------------------------

    /**
     * Creates a minimal Depth with the specified number of bids and asks.
     */
    private Depth createDepth(String symbol, int numBids, int numAsks) {
        Depth d = new Depth();
        d.symbol = symbol;

        d.bids = new java.util.ArrayList<>();
        for (int i = 0; i < numBids; i++) {
            Depth.Line line = new Depth.Line();
            line.price = 100.0 - i; // descending prices: 100, 99, 98...
            line.size = 100 + i * 10;
            d.bids.add(line);
        }

        d.asks = new java.util.ArrayList<>();
        for (int i = 0; i < numAsks; i++) {
            Depth.Line line = new Depth.Line();
            line.price = 101.0 + i; // ascending prices: 101, 102, 103...
            line.size = 200 + i * 10;
            d.asks.add(line);
        }

        d.trades = new java.util.ArrayList<>();
        d.tradeBreaks = new java.util.ArrayList<>();
        return d;
    }

    @Test
    void bookBuilder_initializesOrderBookWithBidsAndAsks() {
        Depth depth = createDepth("TEST", 3, 2);

        OrderBook book = new ExchangeImpl().getOrderBook("TEST");
        ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1);

        try {
            new BooksBuilder.BookBuilder(
                    book, depth, executor,
                    Long.MAX_VALUE, // won't fire
                    0.1, 0.05, 2, 0.0, 10, 0.0
            );

            assertEquals(3, book.getBuyOrders().length,
                    "BookBuilder should populate all 3 bid levels");
            assertEquals(2, book.getSellOrders().length,
                    "BookBuilder should populate all 2 ask levels");

            // Verify buy orders sorted descending by price
            Order[] buyOrders = book.getBuyOrders();
            for (int i = 0; i < buyOrders.length - 1; i++) {
                assertTrue(
                        buyOrders[i].getPrice().compareTo(buyOrders[i + 1].getPrice()) >= 0,
                        "Buy orders should be sorted highest price first"
                );
            }

            // Verify sell orders sorted ascending by price
            Order[] sellOrders = book.getSellOrders();
            for (int i = 0; i < sellOrders.length - 1; i++) {
                assertTrue(
                        sellOrders[i].getPrice().compareTo(sellOrders[i + 1].getPrice()) <= 0,
    ...
    @Test
    void bookBuilder_pricesAreScaledToTickScale() {
        Depth depth = createDepth("SCALE", 1, 0);
        // Override first bid price to one needing rounding
        depth.bids.get(0).price = 10.156; // will be scaled to 2 dp → 10.16

        OrderBook book = new ExchangeImpl().getOrderBook("SCALE");
        ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1);

        try {
            new BooksBuilder.BookBuilder(
                    book, depth, executor, Long.MAX_VALUE,
                    0.1, 0.05, 2, 0.0, 10, 0.0
            );

            Order topBid = book.getBuyOrders()[0];
            BigDecimal expected = new BigDecimal("10.156").setScale(2, RoundingMode.HALF_EVEN);
            assertEquals(expected, topBid.getPrice());
        } finally {
            executor.shutdownNow();
        }
    }
    ...
    @Test
    void bookBuilder_emptyBidsAndAsks_leavesBookEmpty() {
        Depth depth = createDepth("EMPTY", 0, 0);

        OrderBook book = new ExchangeImpl().getOrderBook("EMPTY");
        ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1);
