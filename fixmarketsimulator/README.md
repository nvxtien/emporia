...
Runs as part of the emporia Maven reactor (`mvn -f pom.xml -pl fixmarketsimulator -am ...` from the repo root) but is a standalone app (Guice/Jetty/Jersey/QuickFIX-J, not Spring Boot) with its own groupId/version, not parented to `emporia-services`. Its FIX-protocol and gRPC contract classes come from the [`fix-simulator-contracts`](../fix-simulator-contracts) module, which `market-data-service` also depends on.
